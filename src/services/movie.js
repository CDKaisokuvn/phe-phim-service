...
export async function updateMovieDetail(movie) {
    ...
    movie.categories = $(".info-y li").eq(3).text().split(":")[1];
    movie.slug_cats = (movie.categories && slug(movie.categories)) || "";
    movie.times = $(".info-y li").eq(5).text().split(":")[1];
    movie.views =
        +$(".info-y li")
            .eq(6)
            .text()
            ...
            .replace("'", "")
            .replace('"', "") || 0;
    movie.year = $(".info-y li").eq(7).text().split(":")[1];
    movie.country = $(".info-y li").eq(4).text().split(":")[1];
    movie.actors = $(".info-y li").eq(2).text().split(":")[1];
    movie.director = $(".info-y li").eq(1).text().split(":")[1];
